  ...
  return (
    <g>
      ...
      <text
        ...
        style={{ fontSize: "14px" }}
      >
        {`(Tỷ lệ ${(percent * 100).toLocaleString("vi-VN")}%)`}
      </text>
    </g>
  );
};
...
const HotelPieChart = () => {
  ...
  useEffect(() => {
    const fetchRoomTypes = async () => {
      try {
        const response = await fetch(`${process.env.API}/admin/hotel`);
            ...
            <PieChart>
              ...
              <Tooltip
                ...
                formatter={(value, name, props) => [
                  ...
                  <span>{`${(props.payload.percent * 100).toLocaleString(
                    "vi-VN"
                  )}%`}</span>,
                ]}
              />
